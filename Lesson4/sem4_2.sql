...
FROM clients
GROUP BY login );
-- остались записи с разными суррогатными ключами 
...
-- Error Code: 1055. Expression #1 of SELECT list is not in GROUP BY clause and contains nonaggregated column 'lesson_4.users.id' which is not functionally dependent on columns in GROUP BY clause; this is incompatible with sql_mode=only_full_group_by
(SELECT *
FROM users) 
...
(SELECT *
FROM clients
GROUP BY clients.login); -- группировка для убирания дубликатов. Посмотреть еще способы



-- Оператор IN
SELECT "A" IN ('A','B','C','D') AS result; -- как предикат, возвращает 1 или 0
SELECT *
FROM clients
WHERE login IN('Mikle','Tom','Masha');  -- можно делать SELECT ОДНОГО столбца внутри скобок

-- все логины кроме Микла
SELECT *
FROM clients
