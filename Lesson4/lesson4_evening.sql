...
-- подзапрос 1, фильтруем отдельно друзей user'a  c id = 1
select * from friend_requests fr where (fr.initiator_user_id = 1 or fr.target_user_id = 1) and fr.status = 'approved';
-- подзапрос 2, склейка  сообщений и имен друзей
select * from (select * from friend_requests fr where (fr.initiator_user_id = 1 or fr.target_user_id = 1) and fr.status = 'approved') as t1
	join users u on IF (initiator_user_id = 1 , t1.target_user_id , t1.initiator_user_id ) = u.id
	join messages m2 on m2.from_user_id = u.id where m2.to_user_id = 1;
-- запрос 3, подсчет сообщений от друзей
Select CONCAT(firstname, ' ', lastname) as `top friend` , COUNT(*) as message_count from 
	(select initiator_user_id,target_user_id, `status` from friend_requests fr where (fr.initiator_user_id = 1 or fr.target_user_id = 1) and fr.status = 'approved') as t1
	join users u on IF (initiator_user_id = 1 , t1.target_user_id , t1.initiator_user_id ) = u.id
	join messages m2 on m2.from_user_id = u.id where m2.to_user_id = 1
...
SELECT CONCAT(
	...
	, ' больше всего сообщений! А именно: '
	, z.count_messages 
	) AS 'Результат' 
    
/*
FROM (
	SELECT -- просто считаем количество сообщений, группируем по отправителю
	u.id
	,m.from_user_id
	,COUNT(*) AS count_messages
	FROM users AS u
	JOIN friend_requests AS fr ON u.id = fr.target_user_id AND fr.status = 'approved'  -- подтвержденные друзья
	JOIN messages AS m ON m.from_user_id = fr.initiator_user_id AND m.to_user_id = fr.target_user_id    
	WHERE u.id = 1  -- параметр поиска!!!!
	GROUP BY u.id,m.from_user_id
	ORDER BY count_messages DESC  -- чтобы тот, кто больше всего написал сообщений - был в 1-й строке
	LIMIT 1 
) AS z  -- тут будет только одна строка или пустая таблица (если этому пользователю никто не писал)
LEFT JOIN users AS u1 on u1.id = z.id  -- для получателя сообщений
LEFT JOIN users AS u2 on u2.id = z.from_user_id  -- для отправителя сообщений
;
*/

-- SELECT 
-- 	u1.id = 1,
-- 	CONCAT(u1.firstname, ' ', u1.lastname) AS 'Пользователь',
-- 	m.from_user_id,
-- 	CONCAT(u2.firstname, ' ', u2.lastname) AS 'Друг',
-- 	count(m.id)
-- from messages m
-- join users u1 on m.to_user_id = u1.id
-- join users u2 on m.from_user_id = u2.id
-- join friend_requests fr on u1.id = fr.initiator_user_id
-- join friend_requests fr1 on u1.id = fr1.target_user_id 
-- where fr.status = 'approved'
-- GROUP BY u1.id,  'Пользователь', m.from_user_id
